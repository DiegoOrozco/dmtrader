...
export default function FeedbackModal({
    ...
    const modalContent = (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
            ...
            >
                {/* Header */}
                <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-start bg-slate-50 dark:bg-black/20">
                    <div>
                        <h3 className="text-md font-black text-slate-900 dark:text-white uppercase tracking-wider">{studentName}</h3>
                        <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mt-1">{dayTitle}</p>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="text-right bg-slate-50 dark:bg-[#05070f] border border-slate-200 dark:border-slate-800 px-4 py-2 rounded-xl">
                             <div className={`text-xl font-black ${(grade ?? 0) >= 70 ? 'text-emerald-500 dark:text-emerald-400' : 'text-rose-500 dark:text-rose-400'}`}>
                                {grade !== null && grade !== undefined ? `${grade}/100` : "-/100"}
                            </div>
                            <div className="text-[8px] text-slate-550 dark:text-slate-500 font-black uppercase tracking-wider mt-0.5">Nota Final</div>
                        </div>
                        <button
                            onClick={onClose}
                ...
                <div className="p-8 max-h-[70vh] overflow-y-auto space-y-8 custom-scrollbar">
                    {comment && (
                        <div className="space-y-3">
                            ...
                            <div className="p-5 bg-sky-500/[0.02] rounded-2xl border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 text-xs leading-relaxed italic font-semibold">
                                "{comment}"
                            </div>
                        </div>
                    )}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                {positives.length > 0 ? positives.map((p: string, i: number) => (
                                    <div key={i} className="flex gap-3 p-4 bg-emerald-500/5 rounded-2xl border border-emerald-500/10 text-emerald-800 dark:text-emerald-100/90 text-xs font-semibold">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                {improvements.length > 0 ? improvements.map((p: string, i: number) => (
                                    <div key={i} className="flex gap-3 p-4 bg-amber-500/5 rounded-2xl border border-amber-500/10 text-amber-800 dark:text-amber-100/90 text-xs font-semibold">
                                        <div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-amber-550 dark:bg-amber-500 shrink-0" />
                                        {p}
                                    </div>
                                )) : (
                    ...
                    {isObject && feedback.resumen_codigo && (
                        <div className="space-y-2 pt-4 border-t border-slate-200 dark:border-slate-800">
                            ...
                            <p className="text-[10px] text-slate-600 dark:text-slate-400 font-mono bg-slate-50 dark:bg-[#05070f] p-3 rounded-lg border border-slate-200 dark:border-slate-850 uppercase tracking-tighter">
                ...
                <div className="p-6 bg-slate-50 dark:bg-black/20 border-t border-slate-200 dark:border-slate-800 flex justify-end">
                    <button
                        onClick={onClose}
                        className="px-8 py-3 rounded-xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-800 dark:text-white text-[10px] font-black uppercase tracking-widest transition-all border border-slate-200 dark:border-slate-800 cursor-pointer active:scale-95"
                    >
                        Cerrar Vista
                    </button>
                </div>
