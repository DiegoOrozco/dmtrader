...
export default function AdminPortalLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="flex h-screen bg-slate-50 dark:bg-[#0a0e1a] text-slate-900 dark:text-slate-100 overflow-hidden font-sans">
            {/* Mobile Header Toggle */}
            <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-white dark:bg-[#0a0e1a] border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 z-40 backdrop-blur-md">
                <Link href="/admin" className="flex items-center gap-2 text-slate-900 dark:text-white font-black uppercase tracking-wider">
            ...
            {/* Sidebar Drawer */}
            <aside className={`fixed inset-y-0 left-0 w-64 bg-white dark:bg-[#0a0e1a] border-r border-slate-200 dark:border-slate-800 z-[60] transform lg:relative lg:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col ${isSidebarOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"
                }`}>
                <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-black/20">
                    <Link href="/admin" className="hidden lg:flex items-center gap-3 group">
                        ...
                        <h1 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-widest leading-tight">DM Admin</h1>
                    </Link>
                        ...
                        return (
                            <Link
                                ...
                                className={`flex items-center gap-3 px-4 py-3 text-xs font-black uppercase tracking-wider rounded-xl transition-all border ${isActive
                                    ? "bg-sky-500/10 text-sky-600 dark:text-white border-sky-500/30 shadow-[0_0_20px_rgba(56,189,248,0.1)]"
                                    : "text-slate-500 dark:text-slate-400 border-transparent hover:bg-slate-100 dark:hover:bg-white/[0.02] hover:text-slate-900 dark:hover:text-white"
                                    }`}
                            >
                                {link.icon}
                                <span>{link.name}</span>
                                {link.special && (
                                    <span className="ml-auto text-[9px] font-black uppercase tracking-wider bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-1.5 py-0.5 rounded border border-emerald-500/20">
                                        Live
                                    </span>
                                )}
                            </Link>
                ...
                <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-black/10">
                    <form action={logoutAdmin}>
                        <button type="submit" className="w-full flex items-center gap-3 px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 rounded-xl hover:bg-rose-500/10 hover:text-rose-600 dark:hover:text-rose-400 hover:border-rose-500/20 border border-transparent transition-all text-left">
                            <LogOut size={16} />
            ...
            {/* Main Content Area */}
            <main className="flex-1 relative overflow-y-auto custom-scrollbar pt-16 lg:pt-0 min-h-screen bg-slate-50 dark:bg-[#0a0e1a]">
